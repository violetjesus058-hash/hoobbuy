 ...
 * 2. Replace [Hoobuy Spreadsheet](docs.google.com) -> [Hoobuy Spreadsheet](https://usfanslinki.com/)
...
function processArticle(filePath) {
  ...
  // Step 2: Check for existing Hoobuy Spreadsheet anchor (case-insensitive)
  const ksRegex = /\[(hoobuy spreadsheet)\]\(([^)]+)\)/gi;
  ...
  if (!hasKsAnchor) {
    ...
    const summaryNoLink = summary.replace(
      'visit the [Hoobuy Spreadsheet](https://usfanslinki.com/) homepage',
      'visit the Hoobuy Spreadsheet homepage'
    );
    content = content.trimEnd() + '\n\n' + summaryNoLink + '\n';
  }

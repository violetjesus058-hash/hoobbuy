 ...
 * 2. Replace [Hoobbuy Spreadsheet](docs.google.com) -> [Hoobbuy Spreadsheet](https://usfanslinki.com/)
...
function processArticle(filePath) {
  ...
  // Step 2: Check for existing Hoobbuy Spreadsheet anchor (case-insensitive)
  const ksRegex = /\[(hoobbuy spreadsheet)\]\(([^)]+)\)/gi;
  ...
  if (!hasKsAnchor) {
    ...
    const summaryNoLink = summary.replace(
      'visit the [Hoobbuy Spreadsheet](https://usfanslinki.com/) homepage',
      'visit the Hoobbuy Spreadsheet homepage'
    );
    content = content.trimEnd() + '\n\n' + summaryNoLink + '\n';
  }
